...
#현재 C Node가 부모 노드가 없고 최상위 노드임을 알려주기 위해 'Root Node'의 텍스트를 붙여주고, C Node와 연결되는 그림을 그려주었다.
plt.annotate('level 0 (depth 0)',xy=(47,135),xytext=(20.21,134.3),fontsize='9',arrowprops = dict(arrowstyle="-", color='black', linestyle ='--'))
#현재 level 0 또는 depth 0(깊이)를 나타내는 텍스트를 붙여주고, 1 Node의 현재 깊이가 level 0임을 알려주었다.
plt.annotate('',xy=(63.7,104),xytext=(50.21,134.2),fontsize='9',arrowprops = dict(color='black',arrowstyle="-"))
#밑에 있는 자식 노드인 B를 연결시키기 위해서 실선으로 선을 그려주고 3번 노드와 연결을 시켜주는 그림을 그렸다.
...
#현재 level 1을 나타내는 텍스트를 붙여주고, 2 Node와 3 Node의 깊이를 현재 깊이가 level 1임을 알려주었다.
